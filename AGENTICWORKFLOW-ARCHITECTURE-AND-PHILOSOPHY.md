...
| 5계층 검증 | L0 → L1 → L1.5 → L1.7(pCCS) → L2 | 고위험 도메인: L2 강발현, 연구 도메인: L1.7 강발현 |
...
graph TD
    ...
    B --> C["자식 1: Dissertation Simulator<br/>210-step 박사 논문 / 58 agents"]
    B --> D["자식 2: GlobalNews<br/>20-step 뉴스 분석 / 35 agents"]
    B --> E["자식 N: ...<br/>DNA: 동일 / 도메인별 발현"]
...
graph TB
    subgraph "AgenticWorkflow 코드베이스 (설계 프레임워크)"
        ...
        subgraph "Context Preservation + Safety"
            CPS["hooks/scripts/<br/>63개 프로덕션 + 2개 모듈 + 42개 테스트 스크립트<br/>(Core 4 + pCCS 7 + PD 4 + Quality 22 + Safety 6 + Context 7 + 기타 13)"]
...
**설계 사상:** 서로 다른 전문 영역을 각각 최고 수준으로 깊이 처리하거나, 다관점 분석·교차 검증으로 단일 에이전트보다 풍부한 결과를 얻어야 할 때 사용한다. 이는 **Agent Swarm** 패턴의 구현체로, AI-managed coordination(Team Lead 조율), independent context windows(Teammate 격리 세션), task graph dependencies(Design-time blocks/blockedBy 선언)의 3원칙을 단일 파일 SOT 아키텍처 위에서 실현한다.
...
**Anti-Skip Guard + Verification Gate + pACS + pCCS + Adversarial Review (5계층 품질 보장):**
...
L1.7: pCCS per-claim Confidence (claim 단위 신뢰도 — GroundedClaim 산출물만)
  9. P1 Sandwich: compute_pccs_signals.py(A) → @claim-quality-evaluator(B-1) →
     validate_pccs_assessment.py(C) → @claim-quality-critic(B-2) → generate_pccs_report.py(D)
  10. Claim 유형별 적응 가중치 (FACTUAL:0.50/0.50 → SPECULATIVE:0.15/0.85)
  11. 결정: proceed / rewrite_claims / rewrite_step
  12. pccs-logs/step-N-pccs.md에 기록

L2: Adversarial Review (Enhanced — Review: 필드 지정 단계만)
  13. @reviewer/@fact-checker가 독립적 적대적 검토
  14. P1 검증(validate_review.py)으로 리뷰 품질 보장
  15. review-logs/step-N-review.md에 기록
```
...
## 12. GlobalNews Crawling & Analysis Architecture

### 12.1 개요

AgenticWorkflow의 DNA 유전 철학(§1.4)을 실증하는 두 번째 대규모 자식 시스템이다. 44개 국제 뉴스 사이트 자동 수집 → 56개 NLP 분석 기법 → 5-Layer 신호 분류 → Parquet/SQLite 출력의 뉴스 빅데이터 분석 시스템으로, 부모의 전체 게놈이 뉴스 크롤링·NLP 분석 도메인에 맞게 발현된다.
...
| **에이전트** | 35개 (프레임워크 코어 3 + 도메인 32) |
| **테스트** | 50개 파일, ~2,041 테스트 |
| **워크플로우** | 20-step (Research 4 + Planning 4 + Implementation 12) |

### 12.2 DNA 유전 발현

| 부모 DNA | GlobalNews 발현 형태 |
|---------|---------------------|
| 3단계 구조 | Research (4) → Planning (4) → Implementation (12) |
| SOT 패턴 | `.claude/state.yaml` — Orchestrator만 쓰기 |
| 5계층 QA | L0(a-d) Anti-Skip → Pre-L1 /simplify → L1 → L1.5 pACS → L2 Review(+Focus) |
| P1 봉쇄 | 10개 `validate_*.py` + `diagnose_context.py` = 11개 결정론적 검증 |
| 전문가 위임 | 32개 도메인 에이전트, 6개 에이전트 팀 |
| Safety Hooks | 위험 명령 차단 + 시크릿 출력 감지 + TDD 보호 + 예측적 디버깅 |
| Context Preservation | 스냅샷 + Knowledge Archive + RLM 복원 |

### 12.3 도메인 고유 변이 (부모에 없는 새 유전자)

| 변이 | 설명 |
|------|------|
| **Conductor Pattern** | Claude Code → Python 스크립트 생성 → Bash 실행 → 결과 읽기 (Claude API = $0) |
| **4-Level 재시도** | 90회 자동 시도 + Circuit Breaker + Tier 7 에스컬레이션 |
| **44-site Adapter Pattern** | 사이트별 전용 어댑터 (kr_major 11 + kr_tech 8 + english 12 + multilingual 13) |
| **5-Layer Signal Hierarchy** | Fad → Short → Mid → Long → Singularity (2-of-3 합의) |
| **8-Stage NLP Pipeline** | 전처리 → 피처 → 분석 → 집계 → 시계열 → 교차 → 신호 → 출력 (56개 기법) |
| **Paywall Bypass System** | BrowserRenderer(Patchright) + AdaptiveExtractor(4-stage CSS) + is_paywall_body(26패턴) |
| **Date-Partitioned Storage** | YYYY-MM-DD 디렉터리 구조 — 시계열 분석 전제 |
| **HQ Gates (4종)** | HQ1-HQ4 Human-step 품질 검증 — Autopilot 자동 승인 시 증거 확인 |

### 12.4 아키텍처 (4-Layer)

```
Layer 1: CRAWLING ENGINE   — 44 adapters + anti-block → data/raw/
Layer 2: ANALYSIS PIPELINE — 8 stages, 56 techniques → data/{processed,features,analysis}/
Layer 3: STORAGE           — Parquet ZSTD + SQLite FTS5 → data/output/
Layer 4: PRESENTATION      — Streamlit dashboard (6 tabs) + DuckDB/Pandas/FTS5 쿼리
```

### 12.5 Dissertation Simulator와의 비교

| 차원 | Dissertation Simulator | GlobalNews |
|------|----------------------|------------|
| 도메인 | 학술 연구 시뮬레이션 | 뉴스 빅데이터 분석 |
| 워크플로우 | 210-step (논문 4 Phase) | 20-step (3-Phase) |
| 에이전트 수 | 58개 (10 base + 48 thesis) | 35개 (3 core + 32 domain) |
| SOT | `session.json` (논문 전용) | `state.yaml` (시스템) |
| 산출물 | 마크다운 논문 + GroundedClaim | Parquet + SQLite + 대시보드 |
| 실행 패턴 | 인터랙티브 (HITL 9개) | 배치 (cron 자동화) |
| P1 강발현 | GroundedClaim 47 prefix | 56 NLP 기법 결정론적 파이프라인 |

두 자식 시스템 모두 **동일한 DNA**(절대 기준, SOT, 4계층 QA, Safety Hook, CCP)를 공유하지만, 도메인에 따라 전혀 다른 형태로 발현된다 — DNA 유전 철학(§1.4)의 실증적 검증이다.
